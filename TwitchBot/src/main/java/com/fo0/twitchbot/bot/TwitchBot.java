...
//		bot.setLoginName(name);
	...
	public void connectToTwitch() {
		try {
			bot = TwitchSupport.addSupport(bot);
			bot.getEventManager()
	...
	public void onChatMessage(ChannelMessageEvent event) {
		...
		Logger.debug(String.format("Chat [%s] %s",
		                           msg.getName(),
		                           msg.getMessage()));
	...
	public void onJoin(ChannelJoinEvent event) {
		if (event.getClient()
		         .isUser(event.getUser())) { // It's me!
			// ignore own channel join event!
			Logger.debug(String.format("[IGNORE:Self] User %s joining Channel %s",
			                           event.getActor()
			                                .getNick(),
			                           event.getChannel()
			                                .getName()));
		...
		Logger.debug("User joined: " + event.getUser()
		                                    .getNick());

		if (userJoinListener != null) {
			userJoinListener.accept(event.getUser()
			                             .getNick());
		}
	}
	...
	public void sendMessageToUser(String user, String message) {
		...
		bot.sendMessage(getChannel(),
		                "/w " + user + " " + getFormattedMessage(message));
	}
